...
<div class="p-4 rounded-lg mt-14">
    @if ($teacherTable->count() > 0)
            @foreach ($teacherTable as $teacherClass)
                <h1 class="font-mono font-bold text-2xl text-center text-slate-600 mb-4">Classe: <span class="text-sky-800">{{ $teacherClass->classe->name }}</span></h1>

                <table class="w-full">
                    <thead>
                        <tr class="text-l  font-bold tracking-wide text-left text-slate-600 bg-sky-200 uppercase border border-white">
